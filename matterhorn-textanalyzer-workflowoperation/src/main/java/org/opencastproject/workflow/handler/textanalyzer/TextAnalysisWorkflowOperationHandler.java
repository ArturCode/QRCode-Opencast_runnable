...
public class TextAnalysisWorkflowOperationHandler extends AbstractWorkflowOperationHandler implements ManagedService {
  ...
  protected WorkflowOperationResult extractVideoText(final MediaPackage mediaPackage,
    ...
    //////////////////////////////////////////////
    Track[] t = mediaPackage.getTracks();
    Track sourceTrack = t[0];
    long videoDuration = sourceTrack.getDuration();
    List<Attachment> imagesQr = new LinkedList<Attachment>();
    ...
    try {
      for (long i = 0; i < videoDuration; i += 500) {
        double time = (double)i / 1000;
        extractImageJobsQr.put(i, composer.image(sourceTrack, IMAGE_EXTRACTION_PROFILE, time));
      ...
      for (Map.Entry<Long, Job> entry : extractImageJobsQr.entrySet()) {
        ...
        URI imageUrl = image.getURI();
        long time = entry.getKey() / 1000;
        ...
        String qrcode = QrCodeImp.readQRCode(filePath);
        QrCodeImp.insertDatabase(qrcode, time, mediaPackage.getIdentifier().toString());
        System.out.println("QR-Code: " + qrcode + " from file: " + filePath);
      }
    } catch (EncoderException e) {
      logger.error("Error creating still image(s) from {}", sourceTrack);
      throw e;
